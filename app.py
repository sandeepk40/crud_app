...
def add_users():
    ...
    cursor = conn.cursor()
    sql_query = "INSERT INTO user (name,email,phone,role,password,role_id) VALUES (%s,%s,%s,%s,%s,%s)"
    cursor.execute(sql_query,(name,email,phone,role,password,role_id))
    conn.commit()
...
@app.route('/fetchUserById/<int:id>',methods=['GET'])
def fetch_user_by_idd(id):
     cursor= conn.cursor(dictionary=True)
